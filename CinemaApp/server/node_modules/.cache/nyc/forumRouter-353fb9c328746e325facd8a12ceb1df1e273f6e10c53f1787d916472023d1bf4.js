...
forumRoute.route('/reply/:_id').post((req, res) => {
    Forum.findById(req.params._id).then( res => {
        const oldReplies = res.body.replies
        };
        const newPost ={
            _id: req.params._id,
            name: res.body.name,
            movieName: res.body.movieName,
            rating: res.body.rating,
            message: res.body.message,
            replies: oldReplies.push(req.body.reply)
        };
        Forum.findByIdAndDelete(req.params._id);
        newPost.save().then(() => res.json(newPost)).catch((err) => res.json('Error: ' + err))
